...
function serializePublicSiteSnapshot(snapshot: ReturnType<typeof buildPublicSiteSnapshot>): string {
	...
	return `// Generated by scripts/prepareLocalSite.ts from the active static result summary.\n// Do not edit manually; rerun prepare-local-site after updating local results.\nimport type { PublicSiteSnapshot } from './visualizerData';\n\nexport const publicSiteSnapshot: PublicSiteSnapshot = ${json};\n`;
}
...
const siteSnapshot = buildPublicSiteSnapshot(summary as Parameters<typeof buildPublicSiteSnapshot>[0]);
await writeFile(siteSnapshotUrl, serializePublicSiteSnapshot(siteSnapshot), 'utf8');
console.log('prepare local site: ok copied local-only results into static/results/latest.json and regenerated siteResultSnapshot.ts');
